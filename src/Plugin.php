...
use StageArtPlugIn\Infrastructure\Schema\SurveyMigration;
...
use StageArtPlugIn\Presentation\Admin\SurveyAdmin;
...
use StageArtPlugIn\Presentation\PublicSite\SurveyRouter;
...
final class Plugin{
 public function boot():void{
  ...
  ProductionMigration::ensure();SurveyMigration::ensure();
  ...
  add_action('admin_menu',[new AdminMenu(),'register']);add_action('admin_menu',[new SiteSettingsAdmin(),'register'],20);add_action('admin_menu',[new MemberAdmin(),'register'],20);add_action('admin_menu',[new ProductionAdmin(),'register'],20);add_action('admin_menu',[new SurveyAdmin(),'register'],30);
  ...
  add_action('rest_api_init',static function():void{(new HealthController())->register_routes();(new MemberController())->register_routes();});(new MemberShortcodes())->register();(new MemberRouter())->register();(new ProductionRouter())->register();(new SurveyRouter())->register();
 }
}
